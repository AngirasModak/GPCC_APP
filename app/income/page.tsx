...
export default function Income(){
  ...
  const [incomeTypeFilter,setIncomeTypeFilter]=useState(""); const [categoryFilter,setCategoryFilter]=useState(""); const [flatFilter,setFlatFilter]=useState(""); const [modeFilter,setModeFilter]=useState(""); const [statusFilter,setStatusFilter]=useState("");
  ...
  const filteredRows=useMemo(()=>rows.filter(r=>(!incomeTypeFilter||r.income_type_id===incomeTypeFilter)&&(!categoryFilter||r.income_category_id===categoryFilter)&&(!flatFilter||(r.flat_no||"")===flatFilter)&&(!modeFilter||String(r.mode)===modeFilter)&&(!statusFilter||String(r.status)===statusFilter)),[rows,incomeTypeFilter,categoryFilter,flatFilter,modeFilter,statusFilter]);
  const cleared=filteredRows.filter(r=>String(r.status).toLowerCase()==="cleared"); const total=cleared.reduce((s,r)=>s+Number(r.amount||0),0); const cash=cleared.filter(r=>String(r.mode).toLowerCase()==="cash").reduce((s,r)=>s+Number(r.amount||0),0); const bank=total-cash;
  const rowFlats=Array.from(new Set(rows.map(r=>r.flat_no).filter(Boolean) as string[])).sort();
  const rowModes=Array.from(new Set(rows.map(r=>r.mode).filter(Boolean))).sort(); const rowStatuses=Array.from(new Set(rows.map(r=>r.status).filter(Boolean))).sort();
  const clearFilters=()=>{setIncomeTypeFilter("");setCategoryFilter("");setFlatFilter("");setModeFilter("");setStatusFilter("");};
  ...
  return <>
    ...
    <div className="card record-filter-card"><div className="record-filter-head"><div><h3>Filter Income Records</h3><p className="muted">Narrow the table and summary cards by income classification, flat, receipt mode or status.</p></div><button className="btn secondary small-btn" onClick={clearFilters}>Reset filters</button></div><div className="record-filter-grid">
      <label>Income Type<select className="input" value={incomeTypeFilter} onChange={e=>{setIncomeTypeFilter(e.target.value);setCategoryFilter("")}}><option value="">All Income Types</option>{types.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label>
      <label>Category<select className="input" value={categoryFilter} onChange={e=>setCategoryFilter(e.target.value)}><option value="">All Categories</option>{categories.filter(c=>!incomeTypeFilter||c.income_type_id===incomeTypeFilter).map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
      <label>Flat<select className="input" value={flatFilter} onChange={e=>setFlatFilter(e.target.value)}><option value="">All Flats</option>{rowFlats.map(f=><option key={f} value={f}>{f}</option>)}</select></label>
      <label>Receipt Mode<select className="input" value={modeFilter} onChange={e=>setModeFilter(e.target.value)}><option value="">All Receipt Modes</option>{rowModes.map(m=><option key={m} value={m}>{m}</option>)}</select></label>
    ...
    </div><div className="record-filter-count">Showing <b>{filteredRows.length}</b> of <b>{rows.length}</b> income records</div></div>
    <div className="card tableWrap"><table className="table"><thead><tr><th>Date</th><th>Income Type</th><th>Category</th><th>Contributor</th><th>Flat</th><th>Receipt Mode</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filteredRows.length?filteredRows.map(r=><tr key={r.id}><td>{r.date}</td><td>{typeName(r)}</td><td>{catName(r)}</td><td>{r.contributor}</td><td>{r.flat_no||"-"}</td><td>{r.mode}</td><td>{money(r.amount)}</td><td><span className="status">{r.status}</span></td><td className="actions"><button className="btn secondary small-btn" onClick={()=>edit(r)}>Edit</button><button className="btn danger small-btn" onClick={()=>del(r.id)}>Delete</button></td></tr>):<tr><td colSpan={9} className="empty">No income entries match the selected filters.</td></tr>}</tbody></table></div>
