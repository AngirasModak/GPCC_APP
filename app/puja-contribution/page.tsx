...
export default function PujaContribution(){
 ...
 return <div className="puja-page">
  ...
  <div className="puja-analytics-grid">
   <div className="card puja-chart-card"><div className="puja-card-head"><div><h3>HIG / MIG / LIG Collection Performance</h3><p>Amount plus flat-count visibility</p></div></div><div className="puja-category-strip">{byType.map(x=><div key={x.name}><b>{x.name}</b><span>{num(x.Flats)} flats</span><strong>{num(x.PaidFlats)} paid · {x["Collection %"]}%</strong></div>)}</div><div className="puja-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={byType} margin={{top:26,right:48,left:52,bottom:8}}><XAxis dataKey="name"/><YAxis yAxisId="money" width={62} tickFormatter={axisMoney} label={{value:"Amount (₹)",angle:-90,position:"insideLeft",offset:-28,style:{fill:"#617184",fontSize:11,fontWeight:700}}}/><YAxis yAxisId="flats" orientation="right" width={48} allowDecimals={false} label={{value:"No. of Flats",angle:90,position:"insideRight",offset:26,style:{fill:"#617184",fontSize:11,fontWeight:700}}}/><Tooltip formatter={(v:any,n:any)=>String(n).includes("Flats")?num(Number(v)):money(Number(v))}/><Legend/><Bar yAxisId="money" dataKey="Collected" fill="#0f8f7d" radius={[6,6,0,0]}/><Bar yAxisId="money" dataKey="Outstanding" fill="#ef5b5b" radius={[6,6,0,0]}/><Bar yAxisId="flats" dataKey="Flats" fill="#7c6be8" radius={[6,6,0,0]}><LabelList dataKey="Flats" position="top"/></Bar></BarChart></ResponsiveContainer></div></div>
