...
export default function PujaContribution(){
 ...
 const allRows=useMemo(()=>units.map(u=>{
   ...
   return {...u,standard,discount,availed,net,paid,paidOwner,paidTenant,outstanding,status,source:actualSource,payments,flatCount,priority};
 ...
 return <div className="puja-page">
  ...
  <div className="puja-analytics-grid">
   <div className="card puja-chart-card"><div className="puja-card-head"><div><h3>HIG / MIG / LIG Collection Performance</h3><p>Amount plus flat-count visibility</p></div></div><div className="puja-category-strip">{byType.map(x=><div key={x.name}><b>{x.name}</b><span>{num(x.Flats)} flats</span><strong>{num(x.PaidFlats)} paid · {x["Collection %"]}%</strong></div>)}</div><div className="puja-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={byType} margin={{top:18,right:8,left:-18,bottom:0}}><XAxis dataKey="name"/><YAxis yAxisId="money"/><YAxis yAxisId="flats" orientation="right" allowDecimals={false}/><Tooltip formatter={(v:any,n:any)=>String(n).includes("Flats")?num(Number(v)):money(Number(v))}/><Legend/><Bar yAxisId="money" dataKey="Collected" fill="#0f8f7d" radius={[6,6,0,0]}/><Bar yAxisId="money" dataKey="Outstanding" fill="#ef5b5b" radius={[6,6,0,0]}/><Bar yAxisId="flats" dataKey="Flats" fill="#7c6be8" radius={[6,6,0,0]}><LabelList dataKey="Flats" position="top"/></Bar></BarChart></ResponsiveContainer></div></div>
