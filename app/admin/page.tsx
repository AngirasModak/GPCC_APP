...
type EventMaster = { id:string; name:string; description:string|null; event_type?:string|null; budget?:number|null; start_date:string|null; end_date:string|null; is_active:boolean };
...
export default function AdministrationPage() {
  ...
  const [eventForm, setEventForm] = useState({ name:"", description:"", event_type:"Puja", budget:"", start_date:"", end_date:"", is_active:true });
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [permissionsResult, customRolesResult, customPermissionsResult, banksResult, cashResult, categoriesResult, incomeTypesResult, incomeCategoriesResult, eventsResult, residentialResult, auditResult] = await Promise.all([
        ...
        supabase.from("events").select("id,name,description,event_type,budget,start_date,end_date,is_active").order("start_date", { ascending:false }),
        supabase
  ...
  const submitEventMaster = async () => {
    ...
    setBusy(true); clearFeedback();
    try { const payload={...eventForm,name:eventForm.name.trim(),description:eventForm.description.trim()||null,event_type:eventForm.event_type||"Other",budget:eventForm.budget===""?null:Number(eventForm.budget),start_date:eventForm.start_date||null,end_date:eventForm.end_date||null}; const q=editingEvent?supabase.from("events").update(payload).eq("id",editingEvent):supabase.from("events").insert(payload); const {error}=await q; if(error) throw new Error(error.message); setEventForm({name:"",description:"",event_type:"Puja",budget:"",start_date:"",end_date:"",is_active:true}); setEditingEvent(null); setMessage("Event / Campaign saved."); await loadAll(); } catch(e:any){setError(e?.message||"Unable to save Event / Campaign.");} finally {setBusy(false);}
  };
